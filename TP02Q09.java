...
public class TP02Q09 {
    ...
    private static void writeMatricula(long totalTime, int comparisons) {
        String OUTPUT_FILE_NAME = "matrícula_heapsort.txt";
    ...
    public static void main(String[] args) {
        ...
        HeapSort altura = new HeapSort(arr.size());
        ...
        writeMatricula(totalTime, HeapSort.comparisons);
...
class HeapSort {
    ...
    public void sort() {
        ...
        public void reconstruir(int tamanho, int indice) {
            int maior = indice;
            int esquerda = 2 * indice + 1;
            int direita = 2 * indice + 2;

            if (esquerda < tamanho &&
                (array[esquerda].getAltura() > array[maior].getAltura() || 
                (array[esquerda].getAltura() == array[maior].getAltura() && 
                array[esquerda].getNome().compareTo(array[maior].getNome()) > 0))) {
                maior = esquerda;
            }

            if (direita < tamanho &&
                (array[direita].getAltura() > array[maior].getAltura() || 
                (array[direita].getAltura() == array[maior].getAltura() && 
                array[direita].getNome().compareTo(array[maior].getNome()) > 0))) {
                maior = direita;
            }

            if (maior != indice) {
                swap(indice, maior);
                reconstruir(tamanho, maior);
            }
        }
